...
fn make_copy_cmd(src_file_path: &str, dst_path: &str, ssh_cfg: &SshCfg) -> RvResult<Command> {
    lazy_static! {
        static ref SPACES_RE: Regex = Regex::new(r"(?P<space>[ ]+)").unwrap();
    }
    let remote_file_path = if util::is_relative(src_file_path) {
        format!(
            "{}{}{}",
...
pub fn copy(
    ...
    let mut cmd = make_copy_cmd(remote_src_file_path, dst_path, ssh_cfg)?;
...
fn cmd_output_to_vec(mut cmd: Command, filter_extensions: &[&str]) -> RvResult<Vec<String>> {
...
pub fn ssh_ls(
    ...
    println!(" CMD LS  :  {:?}", cmd);
    cmd_output_to_vec(cmd, filter_extensions)
}
...
fn test_commands() -> RvResult<()> {
    ...
    let cmd_string = format!("{:?}", cmd);
    assert_eq!(
        r#""cmd" "/C" "ssh" "-i" "local/path" "someuser@73.42.73.42" "find" "/the/remote with space/folder""#,
        cmd_string,
    );
...
fn test_outputs() -> RvResult<()> {
    ...
    let mut cmd = Command::new("cmd");
    cmd.args(["/C", "echo", "x.png"]);
    let v = cmd_output_to_vec(cmd, &[".jpg"])?;
    assert_eq!(v.len(), 0);

    let mut cmd = Command::new("cmd");
    cmd.args(["/C", "echo", "x.png"]);
    let v = cmd_output_to_vec(cmd, &[".png"])?;
    assert_eq!(v[0], "x.png");
